defmodule Space do
    ...
    def main(input) do
        sizes = input |> build_tree |> calc_sizes
        total = sizes |> Enum.map(fn ({ size, _ }) -> size end) |> Enum.max

        min_size = 30000000 - (70000000 - total)

        sizes
        |> Enum.filter(fn ({ size, node }) -> size >= min_size && node.size == 0 end)
